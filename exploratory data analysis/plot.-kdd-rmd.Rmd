...
#Exploratory data analysis NSL Dataset
...
Character variables is important for first analysis, because they contains more intuitively values, in order to validate the dataset. In fact, variance value close to zero could be interpreted wrong: someone could be think that any one white process generates values randomly, since the all-numeric variable are uncorrelated.
...
Finally, flag feature is analysed respect types of attack and services. Flag is referred to connections value. For this reason, it, much probability, is directly linked to type of service. However, it is necessary not to neglect types of attack.
```{r}
...
Analyzing the graphs, it is possible to conclude that the dataset is correct. Furthermore, the various graphs show the absence of outliers. Despite all the numerical variables are uncorrelated, the character variables do not highlight any outliers, since the individual observations are very close to each other. So the dataset was not randomly generated, being all the uncorrelated variables, but an attack is independent of all the variables that characterize it. This is due to the consequence that an attack slightly changes its shape to reach its goal.

Finally, the type of service frequency is proposed, since the frequency of the ICMP protocol is not shown perfectly in the previous graphs. Small frequencies of the protocol service could suggest that ICMP is not important for this analysis. Although the use of the ICMP protocol has fewer frequencies, it is important for the following analysis, because a possible threat could be caused by the vulnerability  of this protocol.
```{r}
...
For the analysis of numerical variables, all the histograms of each individual numerical variable are shown. It can be noted that the various distributions of the numerical variables that change according to the type of attack. Thus, it is possible to define that the numerical variables are uncorrelated to each other, in fact their covariance is zero, and they have not been generated by a random process.
```{r}
