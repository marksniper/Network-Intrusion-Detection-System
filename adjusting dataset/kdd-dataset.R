#load dataset
train_KDD <- read.table("kddcup_data_10_percent_corrected.csv",
...
train_KDD_without_dulicated <- train_KDD[ !duplicated(train_KDD), ]
#count dupplicate records in train_KDD_without_dulicated
sum(duplicated(train_KDD_without_dulicated))
#calculate percentage of duplicate records
print(paste0("The percentage of duplicate records is: ", round(100-(nrow(train_KDD_without_dulicated)*100/(nrow(train_KDD))),3)))
#save train_KDD_without_dulicated in csv file
write.csv(train_KDD_without_dulicated,file = "train_KDD_without_dulicated.csv",row.names = F)
